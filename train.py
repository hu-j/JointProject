...
class ModelJT(nn.Module):
    ...
    def forward(self, image_in, image_gt, name, training=True):
        ...
        if small:
            if training:
                _, _, h_in, w_in = image_in.shape
                image_in = self.resize_image(image_in)
                image_gt = self.resize_image(image_gt)
                x_feat, y_feat, image_out, y_out = self.enhan_model(image_in, image_gt)
                torch_resize = Resize([h_in, w_in])
                y_out = torch_resize(y_out)
                image_out = torch_resize(image_out)
                image_gt = torch_resize(image_gt)
                loss_feat = self.feature_matching_loss(x_feat, y_feat)
                loss_fm = self.feature_matching_loss(image_out, y_out.detach())
                loss1_1 = self.restoration_loss(image_out, image_gt)
                loss1_2 = self.restoration_loss(y_out, image_gt)

                losses = [loss_fm, loss1_1, loss1_2]
                restor_loss = 1 * losses[0] + \
                              2 * losses[1] + \
                              2 * losses[2] + 1 * loss_feat
            else:
                _, _, h_in, w_in = image_in.shape
                image_in = self.resize_image(image_in)
                image_gt = self.resize_image(image_gt)
                image_out = self.enhan_model(image_in)
                torch_resize = Resize([h_in, w_in])
                image_out = torch_resize(image_out)
                image_gt = torch_resize(image_gt)
            ...
            if training:
                ...
                losses = [loss_fm, loss1_1, loss1_2]
                # restor_loss = self.weights[0] * losses[0] + \
                #               self.weights[1] * losses[1] + \
                #               self.weights[2] * losses[2] + 0.5 * loss_feat
                restor_loss = 1 * losses[0] + \
                              2 * losses[1] + \
                              2 * losses[2] + 1 * loss_feat
            else:
                image_out = self.enhan_model(image_in)
                # _, _, image_out, y_out = self.enhan_model(image_in, image_gt, not_save=False)
                restor_loss = self.restor_loss(image_out, image_gt)
        ...
        # return x_hat, y, image_out, restor_loss, ssim, psnr
        ...
        if training:
            ...
            # feature vision
            # for f_i, (det_fpn_i, gt_fpn_i) in enumerate(zip(det_fpn, gt_fpn)):
            #     if f_i == 0:
            #         saver.save_feature(det_fpn_i, name[0] + '_fpn_out')
            #         saver.save_feature(gt_fpn_i, name[0] + '_fpn_gt')
            #         fpn_contrast = gt_fpn_i - det_fpn_i
            #         saver.save_feature(fpn_contrast, name[0] + '_fpn_contrast')

            # for f_i, (det_fpn_i, gt_fpn_i) in enumerate(zip(det_head_feat, gt_head_feat)):
            #     if f_i == 0:
            #         saver.save_feature(det_fpn_i, name[0] + '_head_out')
            #         saver.save_feature(gt_fpn_i, name[0] + '_head_gt')
            #         fpn_contrast = gt_fpn_i - det_fpn_i
            #         saver.save_feature(fpn_contrast, name[0] + '_head_contrast')

            # for f_i, (det_dark_i, gt_dark_i) in enumerate(zip(det_dark, gt_dark)):
            #     if f_i == 0:
            #         saver.save_feature(det_dark_i, name[0] + '_dark_out')
            #         saver.save_feature(gt_dark_i, name[0] + '_dark_gt')
            #         fpn_contrast = gt_dark_i - det_dark_i
            #         saver.save_feature(fpn_contrast, name[0] + '_dark_contrast')
            ...
            if not opt.sampling:
                for iter, (data, target, name) in enumerate(progress_bar):
                    ...
                    try:
                        ...
                        image_out, restor_loss, det_loss, \
                        iou_loss, l1_loss, conf_loss, cls_loss, \
                        feature_loss, head_iou_loss, head_cls_loss, head_obj_loss, \
                        psnr, ssim, in_det_outputs, gt_outputs = model(data, target, name)
                        ...
                        loss = restor_loss + feature_losses + head_iou_losses + head_cls_losses + head_obj_losses
            ...
            if epoch % opt.val_interval == 0:
                ...
                for iter, (data, target, name) in enumerate(val_generator):
                    with torch.no_grad():
                        ...
                        image_out, restor_loss, det_outputs, gt_outputs, \
                        restor_loss, \
                        psnr, ssim = model(data, target, name, training=False)
                        ...
                        # feature_losses = feature_loss[0] + feature_loss[1] + feature_loss[2]
                        # loss = restor_loss + feature_losses
                        restor_loss_ls.append(restor_loss.item())
